...
for p=1:Mx
    for q=1:My
        getCoords(lambda,delta,f,R,db,Nx,Ny,p,q,dx,dy,incidentAngle,DownSamplingAccuCtrl,CoordsAccuCtrl,polyPre,polyForm,polyPost);
    end
end
toc
%% generate GDS
...
function B=getCoords(lambda,delta,f,R,db,Nx,Ny,p,q,dx,dy,incidentAngle,AccuCtrl,CoordsAccuCtrl,polyPre,polyForm,polyPost)
lambda=lambda*db;
delta=delta*db;
f=f*db;
% offset=f*tan(incidentAngle);
...
C=CsplitConcave(Bs);
if fliped~=0
    CgetPolygonCoordsUsingAdjacentCoords(C,polyPre,polyForm,polyPost,Ny,p,q,0,0,R,0);
else
...
L(end,1)=L0(end,1);
B=CgetBoundariesFromLabel(L,xs,ys,delta,f,lambda,th,incidentAngle,CoordsAccuCtrl);
% tic
