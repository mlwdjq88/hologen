...
angs(num)=i;
s(num)=interferogram(186,168);
s2(num)=interferogram(168,168);
...
phs(mask~=1)=NaN;
phs=DelTilt(phs);
figure(3),imagesc(x(1,:)*100,y(:,1)*100,phs);

%% fft
ft=fftshift(fft2(interferogram-mean(interferogram(:))));
ft2=zeros(500);
ft2(195:305,195:305)=ft(195:305,259:369);
...
ph4=DelTilt(ph4);
ph4=DelTilt(ph4);
figure(4),imagesc(x(1,:)*100,y(:,1)*100,-ph4);
