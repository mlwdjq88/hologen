...
int CalSlope(double x0,double y0,double x1,double y1,double x2,double y2){
    if (fabs(atan2(y1-y0,x1-x0))>fabs(atan2(y2-y0,x2-x0)))
        return 1;
    else
        return -1;
}
...
//                     D[2*num+1]=D[2*num-7];
                    D[2*num+2]=D[2*num-6];
                    D[2*num+3]=D[2*num-5];
                    pn=CalSlope(D[2*num-5],D[2*num-6],D[2*num-7],D[2*num-8],D[2*num-3],D[2*num-4]);
                    D[2*num]=D[2*num-6-2*pn];
                    D[2*num+1]=D[2*num-5-2*pn];
                    num=num+2;
                }
            }
            else if (t==5){
...
//                     D[2*num+1]=D[2*num-5];
                    D[2*num+2]=D[2*num-4];
                    D[2*num+3]=D[2*num-3];
                    pn=CalSlope(D[2*num-3],D[2*num-4],D[2*num-5],D[2*num-6],D[2*num-1],D[2*num-2]);
                    D[2*num]=D[2*num-4-2*pn];
                    D[2*num+1]=D[2*num-3-2*pn];
                    num=num+2;
                }
            }
        }
        ...
        for( i=(dire-1)*rowLen; i<dire*rowLen-1; i++){
            ...
            if (t==4){
                ...
                if (i!=dire*rowLen-2){
                    ...
                    D[2*num+2]=D[2*num-6];
                    D[2*num+3]=D[2*num-5];
//                     pn=CalSlope(D[2*num-6],D[2*num-5],D[2*num-8],D[2*num-7],D[2*num-4],D[2*num-3]);
// //                     std::cout << std::fixed <<pn<<  '\n';
//                     D[2*num]=D[2*num-6-2*pn];
//                     D[2*num+1]=D[2*num-5-2*pn];
                    num=num+2;
                }
                ...
                if (i!=dire*rowLen-2){
                    ...
                    D[2*num+2]=D[2*num-4];
                    D[2*num+3]=D[2*num-3];
//                     pn=CalSlope(D[2*num-4],D[2*num-3],D[2*num-6],D[2*num-5],D[2*num-2],D[2*num-1]);
// //                      std::cout << std::fixed <<pn<<  '\n';
//                     D[2*num]=D[2*num-4-2*pn];
//                     D[2*num+1]=D[2*num-3-2*pn];
                    num=num+2;
...
//     char * fileName ="test";
    ...
    long Ny       = mxGetScalar(prhs[5]);
    int p       = mxGetScalar(prhs[6]);
    int q       = mxGetScalar(prhs[7]);
    long len;
    long num;
        ...
        if (num>=5){
            ...
            for(int m=0; m<4; m++){
                for (int j=0; j<num/5; j++){
                    ...
                    for (int k=0; k<5; k++){
                        E[16*j+5+2*k+16*num/5*m]=flip[2*m]*C[10*j+2*k];
                        E[16*j+6+2*k+16*num/5*m]=flip[2*m+1]*C[10*j+2*k+1];
