...
%             stopsign=0;
%         end
        getCoords(lambda,delta,f,R,db,Nx,Ny,p,q,dx,dy,fliped,incidentAngle);
    end
end
toc
...
parfor q=1:clen
    ...
    k=1;
    if q==clen
        for t=((q-1)*10+1):length(Ixy0)
            cxy=[Ixy0(t).xr,Ixy0(t).yr];
            cxy=unique(cxy,'rows');
            cn(k)=length(cxy);
            k=k+1;
            cx=[cx;cxy(:,1)];
    ...
    %  B=getBoundariesFromLabel(Ixy0,delta,f,lambda,th,incidentAngle,CoordsAccuCtrl);
    Bs=CdownSamplingUsingRealCoords(B,lambda,delta,f,DownSamplingAccuCtrl);
    C=CsplitConcave(Bs);
    CgetPolygonCoordsUsingAdjacentCoords(C,polyPre,polyForm,polyPost,Ny,1,q,offsetx,offsety,R,Rb);
end
%% generate GDS
tic
CreadCor(1,clen,filename);toc
delete('*.cor');
    end
end
% end
...
function getCoords(lambda,delta,f,R,db,Nx,Ny,p,q,dx,dy,fliped,incidentAngle)
lambda=lambda*db;
delta=delta*db;
f=f*db;
% offset=f*tan(incidentAngle);
...
xs=rs.*cos(TH);
ys=rs.*sin(TH);
getBoundaries(xs,ys,delta,f,lambda,incidentAngle,p,q);




function getBoundaries(xs,ys,delta,f,lambda,incidentAngle,p,q)
...
%      th0
%      r0
    [xr,yr]=getAccurateCoords(xr,yr,x0,y0,delta,f,lambda,th,incidentAngle,CoordsAccuCtrl);
%     if i==19
...
function [xr,yr]=getAccurateCoords(xr,yr,x0,y0,delta,f,lambda,th,incidentAngle,accu)
N=length(xr);
...
xr=xr+x0;
yr=yr+y0;
Index=ri>1.1*rimax|ri<0.9*rimin|abs(ri-r0)./(rimax-rimin)>1/20|abs(ri-r0)./r0>1/4;
% Index2=abs(getIntensity((xr+xr0)/2,(yr+yr0)/2,delta,f,lambda,incidentAngle)-th)>abs(getIntensity(xr0,yr0,delta,f,lambda,incidentAngle)-th);
% abs(getIntensity((xr+xr0)/2,(yr+yr0)/2,delta,f,lambda,incidentAngle)-th)>0.1;
% ri(Index)=[];
% thi(Index)=[];
...
%     yr(by(i))=yb(i);
% end
% xr(Index2)=xr0(Index2);
% yr(Index2)=yr0(Index2);
xr(Index)=[];
yr(Index)=[];
