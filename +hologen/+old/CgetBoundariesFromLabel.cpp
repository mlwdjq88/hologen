...
void getAccurateCoords(double *xr,double *yr,double x0,double y0,double delta,double f,double lambda,double th,double incidentAngle,double CoordsAccuCtrl,long &Ns){
    double *thi, *ri,rimin,rimax,*r0,Ia,Ib,a,b,xs=0,ys=0,Im,s,*xr0,*yr0;
    long Ns0=Ns;
    int k;
    ...
    for (long i=0;i<Ns;i++){
        ...
        if (fabs(Ia)<0.1){
            ...
            k=0;
            while(Im>CoordsAccuCtrl&&fabs(Ia-Ib)>0.0000001&&k<50){
                if (fabs(Ia)>fabs(Ib)){
                    s=(b-a)/(Ib-Ia)*Ib;
                    a=b-s;
...
//     if(i==0)
    for (long i=0;i<Ns;i++){
        if (ri[i]>1.1*rimax||ri[i]<0.9*rimin||fabs(ri[i]-r0[i])/r0[i]>0.5){
            ri[i]=NULL;
            thi[i]=NULL;
        }
    }
    Ns=removeNULL(ri,Ns0);
    Ns=removeNULL(thi,Ns0);
...
void getBoundariesFromLabel(double *L,double *xs,double *ys,double delta,double f,double lambda,double th,double incidentAngle,double CoordsAccuCtrl,int sr,int sc,double* B,long *pB,int &nB){
    double T=lambda/sin(atan(delta/f/2)),cth,cr,tr[4];
    double *xr,*yr,*ths,*rs, th0, r0,x0,y0,thmin,thmax,l,div,*xm,*ym,*thm,*rm,flag;
    xr= (double*) malloc(5*(sr+sc) * sizeof(double));
    yr= (double*) malloc(5*(sr+sc) * sizeof(double));
    ths= (double*) malloc(5*(sr+sc) * sizeof(double));
    rs= (double*) malloc(5*(sr+sc) * sizeof(double));
    xm= (double*) malloc(5*(sr+sc) * sizeof(double));
    ym= (double*) malloc(5*(sr+sc) * sizeof(double));
    thm= (double*) malloc(5*(sr+sc) * sizeof(double));
    rm= (double*) malloc(5*(sr+sc) * sizeof(double));
    long Ns, *Index,*Indexm,*Im,Nm,Ns0,countm;
    int num=dmax(L,0,sr*sc),SubN,angM,direc;
    nB=0;
    pB[0]=0;
    for (int i=1;i<=num;i++){
//         break;
...
//         cr=0;
        ...
        if(5*(sr+sc)<=Ns){
            std::cout << std::fixed <<"allocated array is too small"<<  '\n';
            continue;
        }
//          std::cout << std::fixed <<i<<'\t'<<num<<  '\n';
//         if (i==6)
//             break;
        for(int j=0;j<Ns;j++){
            xr[j]=xs[Index[j]];
...
//         std::cout << std::fixed <<i<<  '\n';
        ...
        getAccurateCoords(xr,yr,x0,y0,delta,f,lambda,th,incidentAngle,CoordsAccuCtrl,Ns);
        
//         if (i==2)
//              for(int j=0;j<Ns;j++){
//             std::cout << std::fixed <<xr[j]<<'\t'<<yr[j]<<'\t' <<Ns<< '\n';}
        if(Ns<3)
            continue;
...
//                     if (fabs(rm[0]-rm[1])>abs(rm[0]-rm[countm-1])){
                ...
                if(j==(SubN-1)&&(cth!=0||cr!=0)){
                    ...
                    B[pB[nB]+2*Ns+1]=cr*sin(cth);
                    pB[nB+1]=pB[nB]+2*(Ns+1);
                    nB++;
                }
            }
        }
...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[])
{
    ...
    B=(double*) malloc((4*(M*N-NZero)-4) * sizeof(double));
    getBoundariesFromLabel(L,xs,ys,delta,f,lambda,th,incidentAngle,CoordsAccuCtrl,M,N,B,pB,nB);
