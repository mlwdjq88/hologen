...
for ex=1:4
   if ex>1
            [kx,ky]=find(uxy(:,5:8)~=0);
            for ks=1:length(kx)
                if uxy(kx(ks),ky(ks))==0
                    uxy(kx(ks),ky(ks))=uxy(kx(ks),ky(ks)+4);
                    uxy(kx(ks),ky(ks)+4)=0;
                    uxy(kx(ks),ky(ks)+8)=uxy(kx(ks),ky(ks)+12);
                    uxy(kx(ks),ky(ks)+12)=0;
                end
            end
   end
...
for t=1:length(Ixy)
    if uxy(t,n)~=0
        k=find(abs(uxy(:,n+1)-uxy(t,n))<0.1&abs(uxy(:,n+9)-uxy(t,n+8))<0.1);
        if ~isempty(k)
            k=k(1);
...
%             else
%                 uxy(t,n)=uxy(t,n+4);
%                 uxy(t,n+8)=uxy(t,n+12);
%                 uxy(t,n+4)=0;
%                 uxy(t,n+12)=0;
%             end
            for l=5:8
                if uxy(t,l)~=0&&uxy(k,l)~=0
                    uxy(t,l-4)=uxy(k,l);
                    uxy(t,l+4)=uxy(k,l+8);
                else
                    uxy(t,l)=uxy(t,l)+uxy(k,l);
                    uxy(t,l+8)=uxy(t,l+8)+uxy(k,l+8);
                end
            end
            if uxy(t,4-n)==0
                uxy(t,4-n)=uxy(k,4-n);
                uxy(t,12-n)=uxy(k,12-n);
            elseif uxy(k,4-n)~=0
                uxy(t,8-n)=uxy(k,4-n);
                uxy(t,16-n)=uxy(k,12-n);
            end
            if uxy(t,5-n)==0
                uxy(t,5-n)=uxy(k,5-n);
                uxy(t,13-n)=uxy(k,13-n);
            elseif uxy(k,5-n)~=0
                uxy(t,9-n)=uxy(k,5-n);
                uxy(t,17-n)=uxy(k,13-n);
            end 
            uxy(k,:)=0;
            uxy(k,:)=0;
            del(count)=k;
...
for t=1:length(Ixy)
    if uxy(t,n)~=0
        k=find(abs(uxy(:,n+5)-uxy(t,n))<0.1&abs(uxy(:,n+13)-uxy(t,n+8))<0.1);
        if ~isempty(k)
            k=k(1);
            Ixy(t).xr=[Ixy(t).xr;Ixy(k).xr];
            Ixy(t).yr=[Ixy(t).yr;Ixy(k).yr];
            ...
            if uxy(t,n+1)~=0&&uxy(k,n+1)~=0
                uxy(t,n+5)=uxy(k,n+1);
                uxy(t,n+13)=uxy(k,n+9);
            else
            uxy(t,n+1)=uxy(t,n+1)+uxy(k,n+1);
            uxy(t,n+9)=uxy(t,n+9)+uxy(k,n+9);  
            end
            if uxy(t,4-n)==0
                uxy(t,4-n)=uxy(k,4-n);
                uxy(t,12-n)=uxy(k,12-n);
...
for t=1:length(Ixy)
    plot(Ixy(t).xr,Ixy(t).yr,'.');hold on;pause(0.1)
end
...
xs=rs.*cos(TH);
ys=rs.*sin(TH);
getBoundaries(xs,ys,delta,f,lambda,incidentAngle,CoordsAccuCtrl,p,q);
return;
% for i=1:length(B)
...
function getBoundaries(xs,ys,delta,f,lambda,incidentAngle,CoordsAccuCtrl,p,q)
global Ixy uxy sns
xynum=length(Ixy);
if xynum==0
Ixy(1).xr=[];
...
for i=1:num
    ...
    temp=find(L(1,:)==i);
    if ~isempty(temp)&&q~=1
        xu(1)=xs((temp(1)-1)*sr+1);
        yu(1)=ys((temp(1)-1)*sr+1);
        sn=sn+1;
        if (temp(end)-temp(1))~=length(temp)-1
            df=find(diff(temp)~=1);
            df=df(1)+1;
            xu(5)=xs((temp(df)-1)*sr+1);
            yu(5)=ys((temp(df)-1)*sr+1);
        end
%     else
%         xu(1)=0;
%         yu(1)=0;
    ...
    temp=find(L(end,:)==i);
    if ~isempty(temp)&&q~=9
        xu(2)=xs((temp(1)-1)*sr+sr);
        yu(2)=ys((temp(1)-1)*sr+sr);
        sn=sn+1;
        if (temp(end)-temp(1))~=length(temp)-1
            df=find(diff(temp)~=1);
            df=df(1)+1;
            xu(6)=xs((temp(df)-1)*sr+sr);
            yu(6)=ys((temp(df)-1)*sr+sr);
        end
%     else
%         xu(2)=0;
%         yu(2)=0;
...
%         yu(3)=0;
    ...
    temp=find(L(:,end)==i);
    if ~isempty(temp)&&p~=9
        xu(4)=xs((sc-1)*sr+temp(1));
        yu(4)=ys((sc-1)*sr+temp(1));
        sn=sn+1;
        if (temp(end)-temp(1))~=length(temp)-1
            df=find(diff(temp)~=1);
            df=df(1)+1;
            xu(8)=xs((sc-1)*sr+temp(df));
            yu(8)=ys((sc-1)*sr+temp(df));
        end
%     else
%         xu(4)=0;
%         yu(4)=0;
...
for i=1:num
[y,x]=find(L==i);
Ixy(xynum+i).xr=xs((x-1)*sr+y);
Ixy(xynum+i).yr=ys((x-1)*sr+y);
end
