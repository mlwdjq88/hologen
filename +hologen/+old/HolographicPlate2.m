...
c(:,(((c(1,:)-N+delta/2/R*N).^2+(c(2,:)-N).^2)>(N+1).^2))=[];
c(:,c(1,:)==0|c(2,:)==0)=[];
% cr=(c(1,:).^2+c(2,:).^2)/R;
% cth=atan2(c(1,:),c(2,:));
% c0(1,:)=cr.*cos(cth);
...
% c0=c;
% figure(3),plot(c0(1,:),c0(2,:),'.');
for i=1:length(c)
    if round(c(1,i))>0&&round(c(2,i))>0
    c1(1,i)=xs(round(c(2,i)),round(c(1,i)))-delta/2;
    c1(2,i)=ys(round(c(2,i)),round(c(1,i)));
    end
end
c2=c1;
c2(1,:)=-c1(1,:);
